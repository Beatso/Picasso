...
client.once("ready", () => {
	...
	;(function startUpdatingStatus() {
		client.user
			.setActivity(`@Picasso | ${client.guilds.cache.size} servers`, {
				type: "WATCHING",
			})
			.catch(console.error) // update status
...
client.on("message", (message) => {
	...
	if (message.reference)
		inputAttachment = message.referencedMessage.attachments.first()
	...
	request.get(inputAttachmentURL, (err, res, inputBuffer) => {
		const scaleFactor = Math.floor(
			512 / Math.min(inputAttachment.width, inputAttachment.height),
		)

		if (scaleFactor < 1)
			message.channel.send(
				"Could not scale the image because it was too large.",
			)
		...
		scalePixelArt(inputBuffer, scaleFactor)
			.then((outputBuffer) => {
				...
				if (Buffer.byteLength(outputBuffer) <= 8000000) {
					...
					message.channel.send(
						"Could not send the scaled image because the file size was too large.",
					)
			})
			.catch((error) =>
				message.channel.send(
					`Scaling the image failed for the following reason:\n\`${error}\``,
				),
			)
	})
})
